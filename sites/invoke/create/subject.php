...
<link rel="stylesheet" href="/gradelens/style/successmessage.css"> <link rel="stylesheet" href="/gradelens/style/errormessage.css">   <script src="/gradelens/script/staticnavigation.js" defer></script>
<body id="background">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Ggf. weitere Felder wie Abkürzung, Beschreibung hier abfragen

    if (empty($subject_name)) {
        // Fehler: Fachname darf nicht leer sein
        ...
        echo "<meta http-equiv='refresh' content='4;url=/GradeLens/sites/invoke/create/subject.php'>";
    } else {
        ...
        if ($check_stmt_global->num_rows > 0) {
            // Fach existiert global, sID holen
            $check_stmt_global->bind_result($existing_sID);
            $check_stmt_global->fetch();
            $sID = $existing_sID;
            $check_stmt_global->close();
        } else {
            // Fach existiert global nicht, also neu anlegen
            $check_stmt_global->close();
            // Hier könnten auch Abkürzung und Beschreibung eingefügt werden, wenn vorhanden
            ...
            if ($stmt_insert_subject->execute()) {
                ...
                // Datenbankfehler beim Einfügen des Fachs
                echo "<link rel='stylesheet' href='/GradeLens/style/errormessage.css'>"; //
        ...
        if ($sID) {
            ...
            if ($user_subject_count > 0) {
                // Benutzer hat dieses Fach bereits zugewiesen
                ...
                // Zuordnung in usersubjects einfügen
                ...
                if ($stmt_usersubjects->execute()) {
                    ...
                    // Datenbankfehler beim Zuordnen des Fachs
                    echo "<link rel='stylesheet' href='/GradeLens/style/errormessage.css'>"; //
            ...
            $conn->rollback();
            echo "<link rel='stylesheet' href='/GradeLens/style/errormessage.css'>"; //
            echo "<section id='errorbackground'><section id='errorcontainer'><p id='errorhead'>UNEXPECTED ERROR</p><section id='errorfoot'><p id='errormessage'>An unexpected error occurred.</p></section></section></section>";
            echo "<meta http-equiv='refresh' content='4;url=/GradeLens/sites/invoke/create/subject.php'>";
        }
    }
}
